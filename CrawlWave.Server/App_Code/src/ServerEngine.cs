...
namespace CrawlWave.Server
{
	...
	public class ServerEngine
	{
		...
		/// </summary>
		/// <param name="ci">The <see cref="ClientInfo"/> of the client returning the data.</param>
		/// <param name="data">An array of bytes containing an array of <see cref="UrlCrawlData"/>
		/// objects containing the data of the crawled urls in serialized form.</param>
		/// <returns>Null if the operation succeeds, or <see cref="SerializedException"/> 
		/// encapsulating the error that occured if the operation fails.</returns>
		public SerializedException StoreCrawlResults(ClientInfo ci, byte [] data)
		{
			SerializedException sx = null;
			try
			{
				...
				UrlCrawlData [] urls = (UrlCrawlData [])sf.Deserialize(ms);
				ms.Close();
				sx = StoreCrawlResults(ci, urls);
			}
			catch(Exception e)
			{
				sx = new SerializedException(e.GetType().ToString(), e.Message, e.ToString());
				if(settings.LogLevel <= CWLogLevel.LogWarning)
				{
					settings.Log.LogWarning("StoreCrawlResults failed: " + e.ToString());
				}
			}
			return sx;
		}
